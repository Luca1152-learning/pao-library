...
public class ManageBorrowsIo {
    public void prompt() {
        while (true) {
            ...
            NumberedOptions options = new NumberedOptions(new ArrayList<>(Arrays.asList(
                    "Give books (" + borrowRequests.size() + " requests)", // 1
    ...
    public void promptGiveBooks() {
        ...
        System.out.println();
        ArrayList<String> stringOptions = new ArrayList<>(borrowRequests.stream()
                .map(borrow -> {
                    ...
                    return "[" + book.getTitle() + " by " + authors + "] to be borrowed by " + user.getFirstName() +
                            " " + user.getLastName();
                }).toList());
    ...
    public void promptReceiveBooks() {
        ...
        // Obtain strings with all the options for the menu
        System.out.println();
        ArrayList<String> stringOptions = new ArrayList<>(borrowsToBeReturned.stream()
                .map(borrow -> {
                    User user = UserService.getUserById(borrow.getUserId());
                    Book book = BookService.getBookById(borrow.getBookId());
                    String authors = String.join(", ", BookService.getBookAuthors(book.getId()));
                    ...
                    return "[" + book.getTitle() + " by " + authors + "] borrowed by " + user.getFirstName() +
